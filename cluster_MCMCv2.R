###Script to partition the phenotypic variance to its components. Can be run iteratively for a specific number of genes on the terminal/cluster.
##For the models with less factors, the corresponding factor is removd from the function variancesCalc
...
variancesCalc <- function(tre, st, end){
  ...
  for (iter in 1:length(colnames(counts))){
    ...
    temp$animal <- rownames(temp)
    temp$dom <- rownames(temp)
    #print(temp$animal)
...
#variances1.2k <- variancesCalc(genes100, start, end)
